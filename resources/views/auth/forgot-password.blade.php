...
<head>
    ...
    <title>Lupa Sandi - FoodSave</title>
...
<body class="antialiased bg-gray-50 text-gray-800 selection:bg-green-500 selection:text-white">
        ...
        <div class="w-full lg:w-1/2 flex items-center justify-center p-8 bg-gradient-to-br from-green-50 to-yellow-50/50">
            <div class="w-full max-w-md glass-panel p-10 rounded-3xl shadow-xl border border-white/60">
                ...
                <div class="mb-8 text-center lg:text-left">
                    <h2 class="text-3xl font-bold text-gray-900 mb-3">Lupa <span class="text-green-700">Kata</span> <span class="text-yellow-500">Sandi?</span></h2>
    ...
    <script>
        ...
        function confirmReset() {
            ...
            if (!email) {
                Swal.fire({
                    icon: 'error',
                    title: 'Oops...',
                    text: 'Silakan masukkan alamat email Anda terlebih dahulu!',
            ...
            Swal.fire({
                ...
                text: "Kami akan mengirimkan tautan pemulihan ke email: " + email,
                icon: "warning",
        ...
        // Show Success Alert if session has status
        @if (session('status'))
            Swal.fire({
                title: "Berhasil!",
                text: "Tautan pemulihan telah dikonfirmasi. Anda akan diarahkan untuk membuat kata sandi baru.",
                icon: "success",
                confirmButtonColor: "#16a34a",
                confirmButtonText: "Atur Ulang Sekarang",
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = "{{ route('password.reset', ['token' => session('reset_token')]) }}?email={{ session('reset_email') }}";
                }
            });
        @endif
        ...
        @if ($errors->any())
            Swal.fire({
                title: "Eror!",
                text: "Terjadi kesalahan. Silakan periksa kembali email Anda.",
                icon: "error",
                confirmButtonColor: "#16a34a",
            });
        @endif
